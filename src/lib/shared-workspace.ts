...
function adminClient(): SupabaseClient | { error: string } {
  ...
  if (!url || !serviceKey)
    return {
      error:
        "Supabase service credentials aren't configured on the server (VITE_SUPABASE_URL / SUPABASE_SERVICE_ROLE_KEY).",
    };
...
async function resolveCaller(
  ...
  if (userErr || !callerEmail)
    return { error: "Not signed in (invalid access token)." };
  ...
  const member =
    (os.members ?? []).find((m) => m.email?.toLowerCase() === callerEmail) ??
    null;
...
const pick = <T extends { id: string }>(
  arr: T[] | undefined,
  allowed: string[],
) => (arr ?? []).filter((x) => allowed.includes(x.id));
const pickMap = <T>(map: Record<string, T> | undefined, allowed: string[]) =>
  Object.fromEntries(
    Object.entries(map ?? {}).filter(([k]) => allowed.includes(k)),
  );
...
export const fetchShared = createServerFn({ method: "POST" })
  ...
  .handler(
    async ({
      ...
      const hqCampaigns = await readKey<Campaign[]>(
        admin,
        hqUid,
        K_CAMPAIGNS,
        [],
      ...
      const hqIds = [
        ...
        ...ids(os.contracts),
        ...ids(hqCampaigns),
        ...extraCollections.flatMap((k) =>
          ids(os[k] as { id: string }[] | undefined),
        ),
      ];
      ...
      const [tasks, assets, budget, events, assignments, allTemplates] =
        await Promise.all([
          ...
          readKey<SharedPayload["assignments"]>(admin, hqUid, K_ASSIGN, {}),
...
export const pushSharedEdit = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }): Promise<{ ok: boolean; error?: string }> => {
    ...
    const { admin, hqUid, os, member } = ctx;
    if (!member)
      return { ok: false, error: "You aren't a member of this workspace." };
    ...
    if (data.kind === "campaignData") {
      ...
      jobs.push(
        patch(K_TASKS, data.tasks),
        patch(K_ASSETS, data.assets),
        patch(K_BUDGET, data.budget),
        patch(K_EVENTS, data.events),
    ...
    if (!((member[listKey] ?? []) as string[]).includes(item.id))
      return { ok: false, error: "Not assigned to you." };
    const list = (os[listKey] ?? []) as { id: string }[];
    const nextOs = {
      ...os,
      [listKey]: list.map((x) =>
        x.id === item.id ? { ...x, ...item, id: x.id } : x,
      ),
    };
