          ...
          >
            <span className="min-w-0">
              <span className="block truncate text-sm font-semibold">
                {active ? active.artist : "No campaign"}
              </span>
              <span className="block truncate text-[11px] text-muted-foreground">
                {active ? active.title : "Create one to begin"}
              </span>
            </span>
            <ChevronDown
              className={`h-4 w-4 shrink-0 transition-transform ${campOpen ? "rotate-180" : ""}`}
            />
          </button>
          {campOpen && (
      ...
      <nav className="flex w-full flex-1 flex-col gap-3 overflow-y-auto pr-1">
        {groups.map((g) => (
          <div key={g.label} className="flex flex-col gap-0.5">
            {!collapsed && (
              <div className="px-3 py-1 text-[9px] font-semibold uppercase tracking-[0.25em] text-muted-foreground/60">
                {g.label}
              </div>
            )}
            {g.items.map((n) => {
        ...
        <Link
          to="/knowledge"
          title="Knowledge Hub"
          className={`group relative flex items-center gap-3 rounded-xl px-3 py-2 text-sm transition-colors ${collapsed ? "justify-center" : ""} ${pathname === "/knowledge" ? "text-white" : "text-muted-foreground hover:text-white"}`}
        >
          {pathname === "/knowledge" && (
            <motion.div
              layoutId="nav-active"
          ...
          <BookOpen className="relative h-4 w-4 shrink-0" />
          {!collapsed && <span className="relative">Knowledge</span>}
        </Link>
        <Link
          to="/settings"
          title="Settings"
