...
/* Interactive DJ / mixer — two decks with real Web Audio: per-deck 3-band EQ,
   volume, tempo (pitch), cue, spinning platter, and an equal-power crossfader.
   Best-effort: if Web Audio isn't available, playback still works (EQ won't). */
...
interface DeckState { trackId: string; playing: boolean; vol: number; low: number; mid: number; high: number; rate: number; }
const initial = (): DeckState => ({ trackId: "", playing: false, vol: 0.8, low: 0, mid: 0, high: 0, rate: 1 });
...
export function DjMixer() {
  ...
  function buildDeck(ctx: AudioContext, el: HTMLAudioElement): DeckNodes {
    ...
    src.connect(low); low.connect(mid); mid.connect(high); high.connect(gain); gain.connect(xfg); xfg.connect(ctx.destination);
  ...
  function ensureEngine() {
    ...
    try {
      ...
      nA.current = buildDeck(ctx, audioA.current);
      nB.current = buildDeck(ctx, audioB.current);
      applyXf(xf);
    } catch { /* web audio unavailable — plain playback still works */ }
  }
  ...
  return (
    <div className="flex flex-col items-center gap-1">
      <div onPointerDown={down} onPointerMove={move} onPointerUp={up} onDoubleClick={() => onChange(unit === "" ? 0.8 : 0)}
        className="relative grid h-12 w-12 cursor-ns-resize place-items-center rounded-full border border-white/15 bg-black/40 touch-none">
