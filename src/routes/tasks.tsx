...
export const Route = createFileRoute("/tasks")({
  head: () => ({ meta: [{ title: "Tasks · RIPPL" }, { name: "description", content: "Release checklist — 6 phases." }] }),
  component: TasksPage,
});
    ...
    <AppShell>
      <header className="glass flex flex-col gap-4 rounded-2xl p-5 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">6-phase rollout — Preparation → Post-Release. Your progress saves automatically.</p>
        </div>
      ...
      <section className="mt-6 grid grid-cols-12 gap-4">
        {releaseChecklist.map((phase) => {
          const done = phase.items.filter((i) => taskState[i.id]).length;
          return (
            <SpotlightCard key={phase.phase} className="col-span-12 md:col-span-6 xl:col-span-4 p-5" spotlight={false}>
              ...
              <div className="mt-4 space-y-1.5">
                {phase.items.map((item) => {
                  const checked = !!taskState[item.id];
                  return (
