...
export const Route = createFileRoute("/invoices")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Commission pipeline, quotes and invoicing.",
      },
    ],
  }),
    ...
    <AppShell>
      <header className="glass rounded-2xl p-5">
        <div className="text-[10px] uppercase tracking-[0.35em] text-white/40">
          Client work · Money in
        </div>
        <h1 className="mt-1 font-display text-3xl font-bold">
          Commissions &amp; <span className="text-gradient-neon">Invoices</span>
        </h1>
        <p className="mt-1 text-sm text-muted-foreground">
          No work without a deposit. Balance invoice on delivery day, every
          time.
        </p>
      </header>
      ...
      <div className="mt-6">
        {tab === "pipeline" ? <Pipeline /> : <Invoices />}
      </div>
    </AppShell>
  );
}
    ...
    <>
      <SpotlightCard className="p-5" spotlight={false}>
        ...
        <form onSubmit={add} className="mt-3 grid grid-cols-12 items-end gap-3">
          <Field
            ...
            label="Client"
            value={f.clientName}
            onChange={(v) => setF({ ...f, clientName: v })}
          />
          <Field
          ...
          <Field
            ...
            type="number"
            value={f.quoteAmount}
            onChange={(v) => setF({ ...f, quoteAmount: v })}
          />
          <Field
            className="col-span-6 sm:col-span-2"
            label="Due"
        ...
        <p className="mt-3 text-[11px] text-muted-foreground/70">
          Quotes expire after 14 days automatically — stated in the generated
          PDF.
        </p>
      </SpotlightCard>
      ...
      {expiringSoon.length > 0 && (
        <div className="mt-4 flex items-center gap-2 rounded-xl border border-[oklch(0.85_0.15_85/0.3)] bg-[oklch(0.85_0.15_85/0.06)] px-4 py-3 text-sm text-[oklch(0.88_0.13_85)]">
          ...
          {expiringSoon.length} quote{expiringSoon.length > 1 ? "s" : ""}{" "}
      ...
      {commissions.length === 0 ? (
        <div className="mt-6">
          <EmptyState
            ...
            note="Log an inquiry to start the pipeline."
          />
        </div>
      ) : (
        <div className="mt-4 space-y-3">
          {commissions.map((c) => {
            ...
            const blocked =
              !c.depositPaid &&
              phaseIdx >= PHASES.indexOf("In Progress") &&
              c.phase !== "Cancelled";
            return (
              <SpotlightCard key={c.id} className="p-5">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div className="min-w-0">
                    ...
                    <div className="text-xs text-muted-foreground">
                      {c.clientName} · {c.kind} · quote valid to{" "}
                      {c.quoteExpiresOn || "—"}
                    </div>
                  </div>
                  <div className="flex shrink-0 items-center gap-2">
                    <select
                      ...
                      onChange={(e) =>
                        patch(c.id, {
                          phase: e.target.value as CommissionPhase,
                        })
                      }
                    ...
                    <button
                      onClick={() => downloadQuote(c)}
                      title="Download quote PDF"
                      className="glass rounded-lg p-2 hover:bg-white/5"
                    >
                ...
                <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
                  <Mini
                    label="Quote"
                    value={mask(money(c.quoteAmount, c.currency))}
                  />
                  <Mini
                    label={`Deposit ${Math.round(c.depositPct * 100)}%`}
                    value={mask(money(deposit, c.currency))}
                  />
                  <Mini
                    label="Revisions"
                    value={`${c.revisionsUsed} / ${c.revisionsIncluded}`}
                  />
                ...
                <div className="mt-3 flex flex-wrap items-center gap-2">
                  <button
                    onClick={() => patch(c.id, { depositPaid: !c.depositPaid })}
                  ...
                  <button
                    onClick={() =>
                      patch(c.id, { revisionsUsed: c.revisionsUsed + 1 })
                    }
                  ...
                  {c.revisionsUsed > c.revisionsIncluded && (
                    <span className="text-[11px] text-[oklch(0.85_0.15_85)]">
                      Over included revisions — raise a new quote.
                    </span>
                  )}
                </div>
                ...
                {blocked && (
                  <div className="mt-3 flex items-center gap-2 rounded-xl border border-[oklch(0.7_0.2_20/0.3)] bg-[oklch(0.7_0.2_20/0.06)] px-3 py-2 text-[11px] text-[oklch(0.8_0.15_20)]">
                    ...
                    Work is in progress without a deposit. That is the rule this
...
function Invoices() {
  ...
  function add(e: React.FormEvent) {
    ...
    const inv: InvoiceRecord = {
      ...
      lines: [
        {
          description: f.description.trim() || "Services",
          quantity: 1,
  ...
  return (
    <>
      <section className="grid grid-cols-12 gap-4">
        <Stat
          label="Outstanding"
          value={mask(`EGP ${Math.round(totals.outstanding).toLocaleString()}`)}
          icon={Wallet}
          accent="oklch(0.85 0.02 260)"
        />
        <Stat
          label="Collected"
          value={mask(`EGP ${Math.round(totals.collected).toLocaleString()}`)}
          icon={Receipt}
          accent="oklch(0.82 0.18 150)"
        />
        <Stat
          label="Overdue"
          value={mask(`EGP ${Math.round(totals.overdue).toLocaleString()}`)}
          icon={AlertTriangle}
        ...
        <Stat
          ...
          value={String(
            commissions.filter((c) => !["Paid", "Cancelled"].includes(c.phase))
              .length,
          )}
          icon={Workflow}
      ...
      {invoices.length === 0 ? (
        <div className="mt-6">
          <EmptyState
            ...
            note="Raise the balance invoice on delivery day, not later."
          />
        </div>
      ) : (
                  ...
                  return (
                    <tr key={i.id} className="border-t border-white/[0.06]">
                      ...
                      <td
                        className="py-3"
                        style={
                          late ? { color: "oklch(0.8 0.15 20)" } : undefined
                        }
                      >
                        {i.dueDate || "—"}
                      ...
                      <td className="py-3 text-right font-mono">
                        {mask(money(t.total, i.currency))}
                      </td>
                      <td className="py-3 text-right font-mono">
                        {mask(money(t.balance, i.currency))}
                      </td>
                      <td className="py-3">
                        <select
                          value={i.status}
