  ...
  return (
    <AppShell>
      <header className="glass rounded-2xl p-5">
        <div className="text-[10px] uppercase tracking-[0.35em] text-white/40">System · Billing</div>
        <h1 className="mt-1 font-display text-3xl font-bold">
          Plans &amp; <span className="text-gradient-neon">Credits</span>
        </h1>
        <p className="mt-1 text-sm text-muted-foreground">
          Metered AI actions with a hard-limited free tier. Stripe checkout plugs in on top.
        </p>
      </header>
      ...
      <SpotlightCard className="mt-6 p-6">
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            ...
            <div className="mt-2 font-display text-4xl font-bold">
              ...
              <span className="text-lg text-muted-foreground">
                {q.limit === 0 ? " credits · unlimited" : ` / ${q.limit} credits`}
              </span>
            </div>
          </div>
          <div className="text-right">
            ...
            <div className="text-[11px] text-muted-foreground">
              {q.plan.priceMonth === 0 ? "Free" : `$${q.plan.priceMonth}/mo`} · {q.plan.seats} seat{q.plan.seats > 1 ? "s" : ""}
            </div>
          </div>
        </div>
          ...
          <>
            ...
            {q.exhausted && (
              <div className="mt-3 rounded-xl border border-[oklch(0.7_0.2_20/0.3)] bg-[oklch(0.7_0.2_20/0.06)] px-3 py-2 text-[11px] text-[oklch(0.8_0.15_20)]">
                Monthly credits exhausted. Metered actions are blocked until the reset or an upgrade.
              </div>
            )}
          </>
        )}
        ...
        {byFeature.length > 0 && (
          <div className="mt-5">
            <div className="text-[10px] uppercase tracking-[0.2em] text-muted-foreground">Where they went</div>
            <div className="mt-2 space-y-1.5">
              {byFeature.map(([feature, credits]) => (
                <div key={feature} className="flex items-center gap-3">
                  <div className="w-36 shrink-0 truncate text-xs">{feature.replace(/_/g, " ")}</div>
                  <div className="h-1.5 flex-1 rounded-full bg-white/[0.06]">
                    <div className="h-full rounded-full bg-white/40"
                      style={{ width: `${Math.min((credits / Math.max(q.used, 1)) * 100, 100)}%` }} />
                  </div>
                  <div className="w-10 shrink-0 text-right font-mono text-[11px] text-muted-foreground">{credits}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        {PLANS.map((p) => {
          ...
          return (
            <SpotlightCard key={p.id} className="col-span-12 p-6 md:col-span-4">
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <div className="mt-2 font-display text-3xl font-bold">
                    ...
                    {p.priceMonth > 0 && <span className="text-sm font-normal text-muted-foreground">/mo</span>}
                  </div>
                </div>
                {current && (
                  <span className="rounded-full bg-white/10 px-2.5 py-1 text-[10px] uppercase tracking-wider">Current</span>
                )}
              </div>

              <div className="mt-4 text-[11px] text-muted-foreground">
                {p.monthlyCredits === 0 ? "Unlimited AI credits" : `${p.monthlyCredits} AI credits / month`}
              ...
              >
                {current ? "Active" : p.priceMonth === 0 ? "Downgrade" : "Choose plan"}
              </button>
      ...
      <div className="mt-4 grid grid-cols-12 gap-4">
        <SpotlightCard className="col-span-12 p-5 md:col-span-6" spotlight={false}>
          <div className="flex items-center gap-2 text-xs uppercase tracking-[0.25em] text-muted-foreground">
          ...
          <div className="mt-3 space-y-1.5">
            {Object.entries(TIER_FEATURES).map(([feature, plans]) => (
              <div key={feature} className="flex items-center justify-between text-sm">
                ...
                <span className={plans.includes(plan) ? "text-[oklch(0.82_0.18_150)]" : "text-muted-foreground"}>
                  {plans.map((p) => PLANS.find((x) => x.id === p)?.name).join(" · ")}
                </span>
              </div>
            ))}
          </div>
          <p className="mt-4 text-[11px] leading-relaxed text-muted-foreground/70">
            Gating is advisory in the UI today — enforce it server-side before you charge anyone.
            See knowledge/checklists/SECURITY_AND_LAUNCH_CHECKLIST.md
          </p>
        </SpotlightCard>
      </div>
      ...
      <SpotlightCard className="mt-4 p-5" spotlight={false}>
        ...
        <ol className="mt-3 space-y-1.5 text-sm text-muted-foreground">
          <li>1. Create three prices in Stripe; put the IDs in <span className="font-mono text-white/70">src/lib/plans.ts</span> and <span className="font-mono text-white/70">public.plans.stripe_price_id</span>.</li>
          <li>2. Add a server function that creates a Checkout session for the signed-in user.</li>
          <li>3. Add a webhook that writes <span className="font-mono text-white/70">public.subscriptions</span> on <span className="font-mono text-white/70">checkout.session.completed</span> and <span className="font-mono text-white/70">customer.subscription.updated</span>.</li>
          <li>4. Keep <span className="font-mono text-white/70">STRIPE_API_KEY</span> and <span className="font-mono text-white/70">STRIPE_WEBHOOK_SECRET</span> server-side only — never as <span className="font-mono text-white/70">VITE_</span> vars.</li>
        </ol>
      </SpotlightCard>
    </AppShell>
  );
