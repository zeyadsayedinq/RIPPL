...
import { KNOWLEDGE_PROMPTS } from "@/lib/knowledge-prompts";
import {
  Rocket,
  Copy,
  Check,
...
export const Route = createFileRoute("/techlab")({
  head: () => ({
    meta: [
      { title: "Tech Lab · RIPPL OS" },
      { name: "description", content: "AI & SaaS project ops." },
    ],
  }),
  component: TechLab,
});
...
const deployColor: Record<string, string> = {
...
function TechLab() {
    ...
    update("projects", (all) =>
      all.map((p) =>
        p.id === pid
          ? {
              ...p,
              tasks: p.tasks.map((t) => (t.id === tid ? { ...t, col } : t)),
            }
          : p,
      ),
    );
      ...
      all.map((p) =>
        p.id === pid
          ? {
              ...
              tasks: [
                ...p.tasks,
                { id: uid("t"), title, col: "Backlog" as SprintCol },
              ],
            }
          : p,
      ),
  ...
  /* Seed the library from knowledge/prompts/* — idempotent, matches on title
     so re-importing never duplicates. */
  const existingTitles = useMemo(
    () => new Set(prompts.map((p) => p.title)),
    [prompts],
  );
  const missing = KNOWLEDGE_PROMPTS.filter((k) => !existingTitles.has(k.title));
  const missingCount = missing.length;
  function importKnowledgePrompts() {
    ...
    <AppShell>
      <header className="glass rounded-2xl p-5">
        ...
        <h1 className="mt-1 font-display text-3xl font-bold">
          AI & <span className="text-gradient-neon">Tech Lab</span>
        </h1>
        <p className="mt-1 text-sm text-muted-foreground">
          Ship your SaaS. Sprint boards + a library of proven AI prompts.
        </p>
      </header>
                    ...
                    {p.tasks
                      ...
                      .map((t) => (
                        <div
                          key={t.id}
                          draggable
                          onDragStart={() => setDrag({ pid: p.id, tid: t.id })}
      ...
      <section className="mt-6">
        <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <button
              onClick={importKnowledgePrompts}
              disabled={missingCount === 0}
              className={`glass inline-flex items-center gap-2 rounded-full px-3.5 py-2 text-sm ${missingCount === 0 ? "cursor-default text-muted-foreground" : "hover:bg-white/5"}`}
            >
              <BookOpen className="h-4 w-4" />
              {missingCount === 0
                ? "Knowledge pack imported"
                : `Import ${missingCount} knowledge prompts`}
            </button>
            <MagneticButton onClick={() => setEnhancer(true)}>
              <Sparkles className="h-4 w-4" /> Enhance a prompt
            </MagneticButton>
          </div>
        </div>
        <div className="space-y-2">
          {prompts.length === 0 && (
            <div className="glass rounded-2xl p-8 text-center text-sm text-muted-foreground">
              No prompts yet. Import the knowledge pack (
              {KNOWLEDGE_PROMPTS.length} ready-made A&amp;R, UGC, pitch and
              legal prompts) or use “Enhance a prompt” to build one.
            </div>
          )}
          {prompts.map((p) => (
            <PromptRow
      ...
      <AnimatePresence>
        {enhancer && <PromptEnhancer onClose={() => setEnhancer(false)} />}
      </AnimatePresence>
    </AppShell>
  );
}
...
> = {
  "Music Generation": {
    ...
    groups: [
      ...
      {
        label: "Tempo",
        options: ["70 BPM ballad", "90 BPM", "110 BPM", "128 BPM dance"],
      },
      {
        label: "Instruments",
        options: [
  ...
  "AI Artist Identity": {
    suffix:
      "Deliver as a cohesive artist bible: persona, palette, sonic signature.",
    groups: [
      {
        label: "Origin",
        options: [
      ...
      {
        label: "Persona",
        options: ["mysterious", "rebellious", "romantic", "regal", "playful"],
      },
    ],
  },
  "Video / Reel Script": {
    ...
    groups: [
      ...
      {
        label: "Tone",
        options: ["raw & real", "aspirational", "funny", "dramatic"],
      },
      {
        label: "CTA",
        options: [
  ...
  "Marketing Copy": {
    ...
    groups: [
      ...
      {
        label: "Emotion",
        options: ["excitement", "trust", "urgency", "belonging"],
      },
      {
        label: "Format",
        options: [
      ...
      {
        label: "CTA",
        options: ["shop now", "learn more", "join the drop", "pre-save"],
      },
    ],
  },
};
...
function PromptEnhancer({ onClose }: { onClose: () => void }) {
  ...
  function surprise() {
    ...
    bank.groups.forEach((g) => {
      ...
      if (Math.random() > 0.6) sel.push(rand(g.options));
    });
    ...
    return (
      ...
      >
        <p className="text-sm text-muted-foreground">
          Your enhanced prompt is in the Prompt Library and synced to Supabase.
        </p>
    ...
    >
      <div className="space-y-4">
                ...
                {g.options.map((o) => {
                  ...
                  return (
                    <button
                      key={o}
                      onClick={() => toggle(o)}
                      className={`rounded-full border px-2.5 py-1 text-[11px] transition-colors ${on ? "border-white bg-white/10 text-white" : "border-white/10 text-muted-foreground hover:text-white"}`}
                    >
                      {o}
                    </button>
                  );
        ...
        <div>
          ...
          <div className="rounded-xl border border-white/10 bg-white/[0.03] p-3 text-sm text-foreground/90">
            {enhanced}
          </div>
        </div>
