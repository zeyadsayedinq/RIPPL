...
export const Route = createFileRoute("/affiliates")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Referral partners, tracked codes and commission payouts.",
      },
    ],
  }),
...
function AffiliatesPage() {
  ...
  function add(e: React.FormEvent) {
    ...
    const code = (
      f.code.trim() || `RIPPL-${f.partnerName.trim().split(/\s+/)[0]}`
    ).toUpperCase();
  ...
  return (
    <AppShell>
      <header className="glass flex flex-wrap items-start justify-between gap-3 rounded-2xl p-5">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Tracked codes, real payouts. An untracked referral is a favour —
      ...
      <section className="mt-6 grid grid-cols-12 gap-4">
        <Stat
          label="Partners"
          value={String(affiliates.filter((a) => a.active).length)}
          icon={Users}
        ...
        <Stat
          ...
          value={mask(`$${Math.round(totals.revenue).toLocaleString()}`)}
          icon={DollarSign}
          accent="oklch(0.82 0.18 150)"
        />
        <Stat
          label="Commission owed"
          value={mask(`$${Math.round(totals.owed).toLocaleString()}`)}
          icon={Share2}
      ...
      <SpotlightCard className="mt-6 p-5" spotlight={false}>
        ...
        <form onSubmit={add} className="mt-3 grid grid-cols-12 items-end gap-3">
          <Field
            ...
            label="Partner name"
            value={f.partnerName}
            onChange={(v) => setF({ ...f, partnerName: v })}
          />
          <Field
            className="col-span-12 sm:col-span-3"
            label="Email"
            value={f.partnerEmail}
            onChange={(v) => setF({ ...f, partnerEmail: v })}
          />
          <Field
        ...
        <p className="mt-3 text-[11px] text-muted-foreground/70">
          Codes must be unique. Duplicate codes are silently rejected so
      ...
      {affiliates.length === 0 ? (
        <div className="mt-6">
          <EmptyState
            ...
            note="Add a partner, share their tracked link, and log conversions here."
          />
        </div>
      ) : (
        <SpotlightCard className="mt-4 p-5" spotlight={false}>
                ...
                {affiliates.map((a) => {
                  ...
                  return (
                    <tr key={a.id} className="border-t border-white/[0.06]">
                      ...
                      <NumCell
                        value={a.clicks}
                        onChange={(v) => patch(a.id, { clicks: v })}
                      />
                      <NumCell
                        value={a.conversions}
                        onChange={(v) => patch(a.id, { conversions: v })}
                      />
                      ...
                      <NumCell
                        value={a.revenue}
                        onChange={(v) => patch(a.id, { revenue: v })}
                        wide
                      />
                      <td className="py-3 text-right font-mono text-[oklch(0.85_0.15_85)]">
                        {mask(`$${owed.toFixed(2)}`)}
                      </td>
                      <td className="py-3">
                        <select
                          value={a.payoutStatus}
                          onChange={(e) =>
                            patch(a.id, {
                              payoutStatus: e.target
                                .value as Affiliate["payoutStatus"],
                            })
                          }
                          className="glass rounded-lg bg-transparent px-2 py-1 text-xs outline-none"
                        >
                          {(["pending", "approved", "paid"] as const).map(
                            (s) => (
          ...
          <p className="mt-4 text-[11px] text-muted-foreground/70">
            Programme-wide conversion rate:{" "}
            <span className="font-mono text-white/80">
              {(totals.cvr * 100).toFixed(1)}%
            </span>
            . Ask for the referral at delivery, never in the same message as an
            invoice.
          </p>
        </SpotlightCard>
      )}
